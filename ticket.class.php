   ...
   **/
   static function getDefaultValues($entity=0) {
      ...
      return  array('_users_id_requester'       => $users_id_requester,
                    ...
                    'due_date'                  => 'NULL',
                    'due_date_PEC'              => 'NULL',
                    'PEC_time_elapsed'          => 'NULL',
                    'PEC_time_allowed'          => 'NULL',
                    'slas_id'                   => 0,
                    '_add_validation'           => 0,
                    'type'                      => $type);
   }
   ...
   **/
   ...
   function showForm($ID, $options=array()) {
      ...
      echo "</td>";
		...
		// Ajout des calculs de SLA
		if($this->fields["slas_id"] > 0) {
			$sla = new SLA();
			$liveSLA = $sla->liveCalculationSLA($ID);
			if($this->fields["status"] == 1) {
				$this->fields["due_date_PEC"] = $sla->computeDueDatePEC($liveSLA,"PEC");
				}
			if(in_array($this->fields["status"], array(2, 3))) {
				$this->fields["due_date"] = $sla->computeDueDatePEC($liveSLA,"Solve");
				}
      ...
      if (!$ID) {
         printf(__('%1$s%2$s'), __('SLA'), $tt->getMandatoryMark('due_date'));
      } else {
         _e('SLA');
      }
      ...
      if ($ID) {
         ...
         echo "</td>";
         echo "<th width='$colsize3%'>".__('Due date PEC')."</th>";
         echo "<td width='$colsize4%'>";
		 if ($this->fields["slas_id"] > 0) {
			if($this->fields["status"] == 1) {
				echo Html::convDateTime($this->fields["due_date_PEC"])." ";
				Html::showToolTip(__('SLA playing'),array("img"=>$CFG_GLPI["root_doc"]."/pics/Button-Play.png"))." ";
				}
			else {
				Html::showToolTip(__('SLA paused'),array("img"=>$CFG_GLPI["root_doc"]."/pics/Button-Pause.png"))." ";
				}
			$t=$liveSLA["pec_time_elapsed"];	$t1=((($t-$t%3600)/3600)<10)?"0".(($t-$t%3600)/3600).":":(($t-$t%3600)/3600).":";	$t1.=(((($t-$t%60)/60)%60)<10)?"0".((($t-$t%60)/60)%60).":":((($t-$t%60)/60)%60).":";	$t1.=(($t%60)<10)?"0".($t%60):($t%60);
			$t=$liveSLA["pec_time"];			$t2=((($t-$t%3600)/3600)<10)?"0".(($t-$t%3600)/3600).":":(($t-$t%3600)/3600).":";	$t2.=(((($t-$t%60)/60)%60)<10)?"0".((($t-$t%60)/60)%60).":":((($t-$t%60)/60)%60).":";	$t2.=(($t%60)<10)?"0".($t%60):($t%60);
			if($t1 > $t2) {Html::showToolTip(__('SLA over'),array("img"=>$CFG_GLPI["root_doc"]."/pics/Button-clock.png"));}
			echo "&nbsp;";
			printf(__('(%1$s used on %2$s allowed)'), $t1, $t2);
			}
         echo "</td>";
         echo "</tr>";
      }
      ...
      if ($ID) {
         ...
         echo "</td>";
         echo "<th width='$colsize3%'>".__('Due date solve')."</th>";
         echo "<td width='$colsize4%'>";
		 if ($this->fields["slas_id"] > 0) {
			if($this->fields["status"] == 2 or $this->fields["status"] == 3) {
			...
			$t=$liveSLA["solve_time_elapsed"];	$t1=((($t-$t%3600)/3600)<10)?"0".(($t-$t%3600)/3600).":":(($t-$t%3600)/3600).":";	$t1.=(((($t-$t%60)/60)%60)<10)?"0".((($t-$t%60)/60)%60).":":((($t-$t%60)/60)%60).":";	$t1.=(($t%60)<10)?"0".($t%60):($t%60);
			$t=$liveSLA["solve_time"];			$t2=((($t-$t%3600)/3600)<10)?"0".(($t-$t%3600)/3600).":":(($t-$t%3600)/3600).":";	$t2.=(((($t-$t%60)/60)%60)<10)?"0".((($t-$t%60)/60)%60).":":((($t-$t%60)/60)%60).":";	$t2.=(($t%60)<10)?"0".($t%60):($t%60);
			if($t1 > $t2) {Html::showToolTip(__('SLA over'),array("img"=>$CFG_GLPI["root_doc"]."/pics/Button-clock.png"));}
			echo "&nbsp;";
			printf(__('(%1$s used on %2$s allowed)'), $t1, $t2);
			}
